...
import { Search, ChevronDown, ChevronRight, ChevronLeft, ArrowUpDown } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { ERP_COLORS } from '@/lib/constants';

/* ── Tag system for "Why This Source" ── */
...
function getFieldTag(fieldName: string): string {
  ...
  if (lower.includes('source') || lower.includes('brand') || lower.includes('category') || lower.includes('pack') || lower.includes('l1') || lower.includes('l2') || lower.includes('l3') || lower.includes('l4') || lower.includes('l5')) return 'Source Priority';
...
function RuleTag({ fieldName }: { fieldName: string }) {
  ...
  return (
    <span className="relative group">
      ...
      {tip && (
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 text-[11px] leading-relaxed text-gray-100 bg-gray-800 dark:bg-gray-700 rounded-lg whitespace-nowrap max-w-[240px] text-wrap z-50 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150 shadow-lg">
          {tip}
...
/* ── Helpers to read fields with newline-containing keys ── */
...
/* ── Business-meaningful fields to show in detail (hide pipeline steps) ── */
...
export function GoldenRecord() {
  ...
  const [tab, setTab] = useState<'viewer' | 'audit'>('viewer');
  ...
  // Silver audit data for audit trail
  ...
  const selectedRecord = expandedIdx !== null ? filtered[expandedIdx] : null;
    ...
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        {isProduct ? (
          ...
          <>
            <MetricCard value="488" label="Golden Records" subtitle="Unique customers" tooltip="Unique entities after deduplication — one record per real-world customer" />
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Search by name, Golden ID, or source ID..."
          className="pl-10"
        />
      </div>
      ...
      {/* Tabs */}
      <TabSwitcher
        tabs={[{ id: 'viewer', label: 'Record Viewer' }, { id: 'audit', label: 'Audit Trail' }]}
        activeTab={tab}
        onTabChange={(id) => setTab(id as 'viewer' | 'audit')}
      />
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {paged.map((rec, i) => {
              ...
              const id = String(getField(rec, idKey));
              const name = String(getField(rec, nameKey));
              const country = countryKey ? String(getField(rec, countryKey)) : '';
              const source = String(getField(rec, sourceKey));
              const quality = String(getField(rec, qualityKey));
              const size = sizeKey ? String(getField(rec, sizeKey)) : '';
              const colCount = (countryKey ? 5 : 4) + (sizeKey ? 1 : 0) + 1;
              ...
              return (
                <React.Fragment key={globalIdx}>
                  ...
                  >
                    <td className={cn('px-4 py-4 font-mono text-xs text-gray-500', isExpanded && 'border-l-[3px] border-l-orange-500')}>{id}</td>
                    <td className="px-4 py-4 font-medium text-gray-900 dark:text-gray-100">{name}</td>
                    {countryKey && <td className="px-4 py-4 text-gray-500">{country}</td>}
                    <td className="px-4 py-4"><ERPBadge system={source} /></td>
                    <td className="px-4 py-4"><StatusBadge status={quality} /></td>
                    {sizeKey && <td className="px-4 py-4 text-right text-gray-500">{size}</td>}
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan={colCount} className="p-0 bg-gray-50/50 dark:bg-slate-800/30 border-b border-gray-100 dark:border-slate-700/50">
                        <div className="px-6 py-5">
                          {tab === 'viewer' ? (
                            <RecordDetail record={rec} isProduct={isProduct} />
                          ) : (
                            <AuditTrail record={rec} idKey={idKey} nameKey={nameKey} silverData={silverData} isProduct={isProduct} sourceKey={sourceKey} />
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 dark:border-slate-700/50">
            ...
            <div className="flex gap-1">
              <Button variant="outline" size="icon" className="h-7 w-7" disabled={page === 0} onClick={() => { setPage(p => p - 1); setExpandedIdx(null); }}>
                <ChevronLeft size={14} />
              </Button>
              <Button variant="outline" size="icon" className="h-7 w-7" disabled={page >= totalPages - 1} onClick={() => { setPage(p => p + 1); setExpandedIdx(null); }}>
...
/* ── Record Detail (expanded below row) ── */
...
function RecordDetail({ record, isProduct }: { record: Record<string, unknown>; isProduct: boolean }) {
  ...
  const fieldEntries = displayFields
    .map(f => {
      ...
      return val !== '' && val !== undefined ? { field: f, value: String(val) } : null;
    })
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        {/* Field table */}
        <div className="xl:col-span-3 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700/60 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Field</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Value</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Source</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">
            ...
            <tbody>
              {fieldEntries.map((entry, i) => {
                const isSystemField = entry.field.toLowerCase().includes('id') || entry.field.toLowerCase().includes('cluster') || entry.field.toLowerCase().includes('quality') || entry.field.toLowerCase().includes('cross') || entry.field.toLowerCase().includes('# source');
                return (
                  <tr key={i} className="border-b border-gray-50 dark:border-slate-700/30">
                    <td className="px-4 py-3 text-xs font-medium text-gray-500">{entry.field}</td>
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100 max-w-[220px] truncate">{entry.value}</td>
                    <td className="px-4 py-3">{isSystemField ? <span className="text-xs text-gray-400">—</span> : <ERPBadge system={primarySource} />}</td>
                    <td className="px-4 py-3"><RuleTag fieldName={entry.field} /></td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="xl:col-span-2 space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700/60 p-5">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4">Record Profile</h3>

            <div className="mb-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                <span>Completeness</span>
                <span>{completeness}/{totalFields} fields</span>
              </div>
              <div className="h-2 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${(completeness / totalFields) * 100}%` }} />
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-1.5 flex-wrap">
                {['SAP', 'Oracle', 'Epicor', 'Infor'].map(erp => (
                  <span
                    key={erp}
                    className={cn('px-2 py-0.5 rounded-full text-[10px] font-medium', linkedSystems.includes(erp) ? 'text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-400')}
                    style={linkedSystems.includes(erp) ? { backgroundColor: ERP_COLORS[erp] } : undefined}
                  >{erp}</span>
                ))}
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700/60 overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr>
              <th className="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Golden ID</th>
              <th className="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">SAP ID</th>
              <th className="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Oracle ID</th>
              <th className="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Epicor ID</th>
              <th className="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Infor ID</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="px-5 py-2.5 font-mono text-xs font-medium text-gray-900 dark:text-gray-100">{String(getField(record, idKey))}</td>
              <td className="px-5 py-2.5 font-mono text-xs text-gray-500">{primarySource === 'SAP' ? String(getField(record, isProduct ? 'Product ID' : 'Source Cust ID')) : '—'}</td>
              <td className="px-5 py-2.5 font-mono text-xs text-gray-500">{primarySource === 'Oracle' ? String(getField(record, isProduct ? 'Product ID' : 'Source Cust ID')) : '—'}</td>
              <td className="px-5 py-2.5 font-mono text-xs text-gray-500">{primarySource === 'Epicor' ? String(getField(record, isProduct ? 'Product ID' : 'Source Cust ID')) : '—'}</td>
              <td className="px-5 py-2.5 font-mono text-xs text-gray-500">{primarySource === 'Infor' ? String(getField(record, isProduct ? 'Product ID' : 'Source Cust ID')) : '—'}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
function AuditTrail({ record, idKey, nameKey, silverData, isProduct, sourceKey }: {
  ...
  const toggleStage = (stage: string) => setExpandedStage(expandedStage === stage ? null : stage);

  return (
    <div className="space-y-4">
      <StageCard stage="BRONZE" title="Raw Ingestion" summary={`Ingested from ${primarySource} as "${recordName}".`} expanded={expandedStage === 'bronze'} onToggle={() => toggleStage('bronze')} color="bg-amber-500">
      ...
      <StageCard stage="SILVER" title="Cleaning Pipeline" summary={auditRow ? `${auditKeys.filter(k => /^S\d/.test(k)).length} transformation steps applied.` : 'Cleaning pipeline applied.'} expanded={expandedStage === 'silver'} onToggle={() => toggleStage('silver')} color="bg-slate-400">
        {auditRow ? (
          <table className="w-full text-sm">
      ...
      <StageCard stage="GOLD" title="Match & Election" summary={`Elected as golden record. ${recordId} assigned.`} expanded={expandedStage === 'gold'} onToggle={() => toggleStage('gold')} color="bg-yellow-500">
  ...
  return (
    <div className="bg-white dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700/60 rounded-xl overflow-hidden">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr>
            <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">GOID</th>
            <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Unit</th>
            <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">Level</th>
            <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 dark:border-slate-700/50">ERP</th>
          </tr>
        </thead>
        <tbody>
